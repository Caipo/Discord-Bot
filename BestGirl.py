...
db = mysql.connector.connect( host="localhost", user="root", password="Starcraft2", database="best_girl")
...
def get_image(name):
    ...
    cursor.execute(f"select * from links where name = \'{name}\'; ")

    urls = { i[0] : i[1::] for i in cursor}
    picked  = random.choice( list(urls.keys()) )
...
async def poll(message):
    ...
    if( message.channel.name == "animemes"  and message.author.id == 309512449315307530 and message.content == "bestgirl" ):
        count = 0
        ...
        while True:
                ...
                while True:
                        ...
                        try: m2 = await message.channel.fetch_message(m2.id)
                        ...
                        if m1.reactions[0].count >= 4:
                                ...
                                cursor.execute( "update anime_girls set number_wins = number_wins + 1 where contender_name = \"" + p1 + "\";")
                        ...
                        if m2.reactions[0].count >= 4:
                                ...
                                cursor.execute( "update anime_girls set number_wins = number_wins + 1 where contender_name = \"" + p2 + "\";")
                        ...
                        if flag:
                            ...
                            cursor.execute( "update anime_girls set elo = " + str(eloLeague.ratingDict[p1]) + " where contender_name = \"" + p1 + "\";")
                            cursor.execute( "update anime_girls set elo = " + str(eloLeague.ratingDict[p2]) + " where contender_name = \"" + p2 + "\";")
                            cursor.execute( "update anime_girls set number_matches = number_matches + 1 where contender_name = \"" + p1 + "\";")
                            cursor.execute( "update anime_girls set number_matches = number_matches + 1 where contender_name = \"" + p2 + "\";")


                            

                            
                            sql_command     =    f'''insert into rounds (contender_A, contender_B, elo_A, elo_B, caipo, squid_cat, slifyre, radscorpion, bake_it_jake, other)
                                                values( \"{p1}\", \"{p2}\", {data[p1][1]}, {data[p2][1]}, \"{votes["Capio"]}\",  \"{votes["SquidCat"]}\",
                                                 \"{votes["Slifyre"]}\",   \"{votes["Radscorpion"]}\", \"{votes["Lary"]}\", \"{votes["other"]}\" ); '''                   
